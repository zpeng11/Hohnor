...
{
	vector<TimeZoneInfo::Transition> transitions;
	vector<TimeZoneInfo::Localtime> localtimes;
	vector<string> names;
	string abbreviation;
};
...
{
	if (!TimeZoneInfo::readTimeZoneFile(zonefile, data_.get()))
	{
		data_.reset();
	}
}
...
TimeZone::TimeZone(int eastOfUtc, const char *name)
	: data_(new TimeZone::Data)
{
	data_->localtimes.push_back(TimeZoneInfo::Localtime(eastOfUtc, false, 0));
...
struct tm TimeZone::toLocalTime(time_t seconds) const
{
	...
	TimeZoneInfo::Transition sentry(seconds, 0, 0);
	const TimeZoneInfo::Localtime *local = findLocaltime(data, sentry, TimeZoneInfo::Comp(true));
...
time_t TimeZone::fromLocalTime(const struct tm &localTm) const
{
	...
	TimeZoneInfo::Transition sentry(0, seconds, 0);
	const TimeZoneInfo::Localtime *local = findLocaltime(data, sentry, TimeZoneInfo::Comp(false));
	if (localTm.tm_isdst)
	{
...
struct tm TimeZone::toUtcTime(time_t secondsSinceEpoch, bool yday)
{
	...
	TimeZoneInfo::fillHMS(seconds, &utc);
